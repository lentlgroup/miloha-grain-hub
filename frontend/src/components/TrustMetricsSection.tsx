      ...
      <div className="container mx-auto px-4">
        <div className="surface-panel-dark rounded-[2.2rem] px-6 py-8 text-secondary-foreground md:px-8">
          <div
            ref={headRef}
            className={cn(
              "mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between reveal-fade",
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {values.map((metric, index) => {
              ...
              return (
                <div
                  id={`trust-metric-${index + 1}`}
                  key={metric.label.en}
                  className={cn(
                    "rounded-[1.6rem] border border-white/10 bg-white/7 p-5 reveal-fade",
                    isVisible && "is-visible",
                  )}
                  style={{ transitionDelay: `${index * 100}ms` }}
                >
                  <div className={cn("mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl", colorClass)}>
                    <Icon size={22} />
                  </div>
